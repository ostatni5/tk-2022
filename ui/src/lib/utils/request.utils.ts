...
export const sendRequest = (request: ImagesRequest, callback: (response: ImagesResponse) => void) => {
    ...
    .then(() => {
        const arrayBuffer = chunks.buffer.slice(chunks.byteOffset, chunks.byteLength + chunks.byteOffset)
