...
std::vector<std::string> processJsonRequestParallel(const cv::FileStorage& cascadeFile,
    ...
    for(int i = 0; i < data.size(); i++){
        std::move(paths.begin() + i*elements, paths.begin() + (i+1)*elements, std::back_inserter(data[i]));
    }
    std::move(paths.begin() + data.size()*elements, paths.end(), std::back_inserter(data[0]));
    ...
    for(int i = 0; i < futures.size(); i++){
        auto resultPart = futures.at(i).get();
